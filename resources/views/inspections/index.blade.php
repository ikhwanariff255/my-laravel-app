    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('inspection.create') }}"
                class="px-4 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 shadow-sm transition-colors flex items-center">
        ...
        @if (session('success'))
            <div
                class="mb-6 p-4 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-700 rounded-lg text-sm flex items-center">
                ...
                <form action="{{ route('inspection.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                    ...
                    <div class="flex-1 relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Search project name, client name or address..."
                    ...
                    <div class="sm:w-48">
                        <select name="type" onchange="this.form.submit()"
                            ...
                            <option value="Kondominium" {{ request('type') == 'Kondominium' ? 'selected' : '' }}>Condominium
                            </option>
                            <option value="Komersial" {{ request('type') == 'Komersial' ? 'selected' : '' }}>Commercial
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="flex gap-2">
                        <button type="submit"
                            class="px-5 py-2.5 bg-slate-800 text-white text-sm font-semibold rounded-xl hover:bg-slate-900 transition-colors shadow-sm">
                            Search
                        </button>

                        @if (request('search') || request('type'))
                            <a href="{{ route('inspection.index') }}"
                                class="px-4 py-2.5 bg-red-50 text-red-600 text-sm font-semibold rounded-xl hover:bg-red-100 transition-colors flex items-center justify-center">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                        @forelse($inspections as $index => $item)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="py-4 px-6 font-medium text-gray-400">{{ $inspections->firstItem() + $index }}
                                </td>

                                <td class="py-4 px-6">
                                    @if ($item->image_url)
                                        <img src="{{ $item->image_url }}" alt="Rumah"
                                            class="w-16 h-12 object-cover rounded-lg shadow-sm border">
                                    @else
                                ...
                                <td class="py-4 px-6">
                                    <a href="{{ route('inspection.show', $item->id) }}"
                                        class="font-bold text-gray-900 hover:text-blue-600 transition-colors">{{ $item->title }}</a>
                                    <span
                                        class="inline-block mt-1 px-2.5 py-0.5 bg-blue-50 text-blue-700 text-xs font-medium rounded-full">
                                        {{ $item->type }}
                                    </span>
                                </td>
                                ...
                                <td class="py-4 px-6 text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <!-- Edit Button -->
                                        <a href="{{ route('inspection.edit', $item->id) }}"
                                        ...
                                        <!-- Delete Button -->
                                        <form action="{{ route('inspection.destroy', $item->id) }}" method="POST"
                                            class="inline-block"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-12 text-center text-gray-400">
                                    ...
                                    <span class="text-base font-medium text-gray-500">No inspection project records
                                        found.</span>
                                    @if (request('search') || request('type'))
