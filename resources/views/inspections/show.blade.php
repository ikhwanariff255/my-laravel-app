        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex gap-2">
                <a href="{{ route('inspection.edit', $inspection->id) }}"
                    class="px-4 py-2 bg-amber-500 text-white text-sm font-semibold rounded-lg hover:bg-amber-600 shadow-sm">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1"><span
                            class="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-full">{{ $inspection->type }}</span>
                    </p>
                </div>
                <div>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <p class="text-base text-gray-700 mt-1 bg-gray-50 p-4 rounded-xl border border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                <div>
                    <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">House Image (Front
                        View)</span>
                    @if ($inspection->img)
                        <img src="{{ $inspection->image_url }}" alt="House Image"
                ...
                <div>
                    ...
                    @if ($inspection->layout_img)
                        <img src="{{ $inspection->layout_url }}" alt="Layout Plan"
                            class="w-full h-64 object-cover rounded-xl shadow-sm border">
                    @else
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Total defects recorded: <span
                            class="font-bold text-blue-600">{{ $inspection->defects->count() }}</span></p>
                </div>
                <a href="{{ route('defects.rapid', $inspection->id) }}"
                    class="px-4 py-2 bg-emerald-100 text-emerald-700 hover:bg-emerald-200 font-bold rounded-lg text-sm transition-colors whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                        @forelse($defects as $index => $defect)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="py-4 px-6">
                                    <span
                                        class="block text-xs font-semibold text-gray-500 uppercase">{{ $defect->category }}</span>
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex gap-1 flex-wrap min-w-max">
                                        {{-- Use the new image_urls accessor instead of checking the raw img column --}}
                                ...
                                <td class="py-4 px-6 text-center">
                                    <div class="flex flex-col sm:flex-row justify-center items-center gap-2 min-w-max">
                                        ...
                                        <a href="{{ route('defects.edit', $defect->id) }}"
                                            class="w-full sm:w-auto px-3 py-1.5 bg-amber-50 text-amber-600 hover:bg-amber-100 rounded text-xs font-bold transition-colors block text-center">
                                            Edit
                                        </a>
                                        ...
                                        <form action="{{ route('defects.destroy', $defect->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="w-full px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded text-xs font-bold transition-colors">
                                                Delete
                                            </button>
                                        </form>
                                    </div>
