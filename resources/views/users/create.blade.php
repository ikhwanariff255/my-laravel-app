...
@section('content')
    <div class="max-w-2xl mx-auto mt-8">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <form action="{{ route('users.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    @error('name')
                        <p class="mt-1.5 text-sm text-red-600 font-medium"><i
        ...
        @else border-gray-300 focus:border-blue-500 focus:ring-blue-200 @enderror">
                        ...
                        {{-- <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>Admin (Super Admin)</option>
                        <option value="company_admin" {{ old('role') == 'company_admin' ? 'selected' : '' }}>Company Admin --}}
                        </option>
                    </select>
                    @error('role')
                        <p class="mt-1.5 text-sm text-red-600 font-medium"><i
                ...
                <div class="pt-6 flex justify-end gap-3 border-t border-gray-100">
                    <a href="{{ route('users.index') }}"
                        class="px-5 py-2.5 bg-gray-100 text-gray-700 hover:bg-gray-200 rounded-lg font-bold transition-colors">Cancel</a>
                    <button type="submit"
                        class="px-5 py-2.5 bg-blue-600 text-white hover:bg-blue-700 rounded-lg font-bold shadow-sm transition-colors">Save
                        User</button>
                </div>
            </form>
        </div>
